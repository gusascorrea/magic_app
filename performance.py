        ...
        for ativos_na_carteira in combinacao_ativos_na_carteira:
            for vol_min in volumes:
                ...
                if estrategia == 'Earnings Yield':
                    sorted_df = df.sort_values(by=['EV/EBIT', 'Liq.2meses'], ascending=[True, False])
                    ...
                    sorted_df = df.sort_values(by=['Magic Formula', 'Liq.2meses'], ascending=[True, False])
                elif estrategia == 'ROIC':
                    sorted_df = df.sort_values(by=['ROIC', 'Liq.2meses'], ascending=[False, False])
                ...
                if not carteira_anterior.empty:
                    ...
                    valor_vendido = carteira_anterior.loc[carteira_anterior.index.isin(vendidos), 'Valor'].sum()
                ...
                sorted_df['Quantidade'] = round((valor_total_atualizado / ativos_na_carteira) / sorted_df['Cotação'], 0)
                sorted_df['Valor'] = sorted_df['Quantidade'] * sorted_df['Cotação']
